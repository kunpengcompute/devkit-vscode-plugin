 ...
 * @param userPath 用户路径
 */
function hasUserFile(userPath: string) {
    try {
...
function hasELength(userPath: string) {
    try {
        const data = fs.readdirSync(`${userPath}/.e`);
        // 如果文件数不是3 个 说明已损坏
...
function hasFData(userPath: string) {
    try {
        const data = fs.readdirSync(`${userPath}/.e/.f`);
        // 如果文件数不是1 个 说明已修改
...
function hasSData(userPath: string) {
    try {
        const data = fs.readdirSync(`${userPath}/.e/.s`);
        // 如果文件数不是1 个 说明已修改
        if (data.length !== 1) { return 'err'; }
...
function isOneDataTrue(userPath: string) {
    try {
        const data = fs.readFileSync(`${userPath}/.e/.f/${ONE_ROOT_FILE}`, 'utf-8');
...
function isTwoDataTrue(userPath: string) {
    try {
        const data = fs.readFileSync(`${userPath}/.e/.s/${TWO_ROOT_FILE}`, 'utf-8');
        if (data.length !== 128) { return 'err'; }
 ...
 * @param userPath 用户路径
 */
function handleErrScene(userPath: string) {
    // 是否存在用户目录
    if (hasUserFile(userPath)) {
        // 判断 .e 目录是否异常
