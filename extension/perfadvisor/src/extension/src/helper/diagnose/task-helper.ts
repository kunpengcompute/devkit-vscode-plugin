 ...
 * 内存诊断任务Task信息处理类
 */
export class DiagnoseTaskHelper {
     ...
     * @param context 插件上下文
     */
    public static async deleteDiagnoseTask(taskName: string | undefined, context: vscode.ExtensionContext, taskId: any) {
        // 校验Task信息
                ...
                if (select === i18n.plugins_sysperf_button_confirm) {
                    try {
                        ...
                        if (resp.code === constant.HTTP_STATUS_CODE.SYSPERF_SUCCESS) {
                            ...
                            // 关闭该任务下所有打开面板
                            DiagnoseTaskHelper.closeTaskRelatedPinal(taskName);
                            ...
                            if (result === i18n.plugins_common_term_view) {
                                this.getImportExportTasksDetail(context, currentLang, taskInfo.taskId).then((resp: any) => {
                                    if (resp.data.data.is_delete) {
                                        ToolPanelManager.openDiagnoseSettingPanel(context, {
    ...
    private static getImportExportTasksDetail(context: vscode.ExtensionContext, language: string, taskId: number) {
        const option = {
            method: 'GET',
            url: `/import_export_tasks/?language=${language}&id=${encodeURIComponent(taskId)}&analysis-type=memory_diagnostic`
        };
    ...
    private static async uploadTaskFile(
        ...
        for (let i = 0; i < length; i++) {
            ...
            const option = {
                method: 'POST', url: '/import_export_tasks/index/?analysis-type=memory_diagnostic',
    ...
    private static async createImportTask(context: vscode.ExtensionContext, params: any): Promise<number> {
        const option = { method: 'POST', url: '/import_export_tasks/import_task/?analysis-type=memory_diagnostic', params };
        const resp: any = await Utils.requestData(context, option, constant.PERF_SUBMODULE.TOOL_SYSPERF_ADVISOR);
        if (resp.status !== constant.HTTP_STATUS.HTTP_200_OK) {
    ...
    private static async createExportTask(context: vscode.ExtensionContext, params: any): Promise<number> {
        const option = { method: 'POST', url: '/import_export_tasks/export/?analysis-type=memory_diagnostic', params };
        const resp: any = await Utils.requestData(context, option, constant.PERF_SUBMODULE.TOOL_SYSPERF_ADVISOR);
        if (resp.status !== constant.HTTP_STATUS.HTTP_200_OK) {
            vscode.window.showErrorMessage(resp.data.message);
            throw new Error(resp.data.message);
    ...
    public static async exportTask(context: vscode.ExtensionContext, operation: 'export' | 'retry', taskInfo: {
        ...
        if (operation === 'export') {
            ...
            if (resp === i18n.plugins_sysperf_continueExport) {
                ...
                const faq = vscode.Uri.parse(pluginUrlCfg.exportTaskUserGuideUrl);
    ...
    public static downloadTaskFile(taskId: any, filesNum: any, filename: any, context: vscode.ExtensionContext) {
        return new Promise((resolve, reject) => {
            ...
            const download = () => {
                ++sectionIndex;
                const option = {
                    url: '/import_export_tasks/download/?analysis-type=memory_diagnostic&id='
                        + encodeURIComponent(taskId) + '&section=' + sectionIndex,
     ...
     * @param context 插件上下文
     */
    public static async runDiagnoseTask(
        projectName: string,
        ...
        if (resp.code === constant.HTTP_STATUS_CODE.SYSPERF_SUCCESS && resp.data.status === constant.STATUS_SUCCESS) {
            try {
                // 调用分析任务接口
                option = {
                    url: `/memory-tasks/${selfInfo.id}/status/`,
                    method: 'PUT',
     ...
     * @param context 插件上下文
     */
    public static async stopDiagnoseTask(context: vscode.ExtensionContext, selfInfo: any) {
        try {
            // 调用停止任务接口
            const option = {
                url: `/memory-tasks/${selfInfo.id}/status/`,
