...
export class LogManager {
    ...
    static async init(context: vscode.ExtensionContext) {
        ...
        log4js.configure({
            appenders: {
                sysPerf: {
                    type: 'file',
                    filename: extensionPath + 'sysPerf.log',
                    maxLogSize: MAX_SIZE,
                ...
                javaPerf: {
                    type: 'file',
                    filename: extensionPath + 'javaPerf.log',
                    maxLogSize: MAX_SIZE,
                    alwaysIncludePattern: true,
                    backups: 5, // keep five backup files
                    encoding: 'utf-8',
            ...
            categories: {
                default: { appenders: ['sysPerf'], level: 'debug' },
                sysPerf: { appenders: ['sysPerf'], level: 'debug' },
                javaPerf: { appenders: ['javaPerf'], level: 'debug' },
            }
        });
    }
    ...
    static async log(context: vscode.ExtensionContext, content: string, module: string, level: LOG_LEVEL) {
        ...
        extensionPath = path.join(context.extensionPath, 'log/');
        const sysLogPath = path.join(extensionPath, 'sysPerf.log');
        const javaLogPath = path.join(extensionPath, 'javaPerf.log');
        if (!fs.existsSync(sysLogPath) || !fs.existsSync(javaLogPath)) {
            this.init(context);
        }
