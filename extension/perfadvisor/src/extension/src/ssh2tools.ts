...
export class SSH2Tools {
    ...
    // deploy_sysPerf脚本
    private DEPLOY_SYSPERF: string[] = ['#!/bin/bash',
        'function help_info(){',
    ...
    // write_sysPerf_log文件
    private WRITE_SYSPERF_LOG: string[] = ['#!/bin/bash',
        'function help_info(){',
    ...
    // uninstall_sysPerf脚本
    private UNINSTALL_SYSPERF: string[] = ['#!/bin/bash',
        'source /etc/profile',
        'cd `dirname $0`',
        'path=`pwd`',
        'function Logger(){',
    ...
    // sysPerf_log文件
    private SYSPERF_LOG: string[] = ['#!/bin/bash',
        'path=$2',
        'function Logger(){',
        'if [ "Error" == $1 ]; then',
        'clean',
    ...
    // upgrade_sysPerf脚本
    private UPGRADE_SYSPERF: string[] = ['#!/bin/bash',
        'function help_info(){',
    ...
    private UPGRADE_SYSPERF_LOG: string[] = ['#!/bin/bash',
        'function help_info(){',
    ...
    public tailFlow(path: string, listener: (data: string) => void) {
        return new Promise((resolve, reject) => {
            this.conn.exec(`touch ${path} && chmod 600 ${path} && tail -f ${path}`, { pty: true }, (err, clientChannel) => {
                if (err) { reject(err); }
