    ...
    static async downloadFile(global: any, data: any) {
        ...
        try {
            ...
            if (data.contentType === 'arraybuffer') {
                await fs.writeFile(file.fsPath, Buffer.from(data.fileContent), fileWriteCallback);
            } else if (data.contentType === 'base64') {
                await fs.writeFile(file.fsPath, Buffer.from(data.fileContent, 'base64'), fileWriteCallback);
            } else {
