...
const messageHandler: { [key: string]: any } = {
    ...
    navigate(message: any) {
        if (message.data.webSession) {
            ...
            ((top as any).webviewSession || {}).setItem('workspace', webSession.workspace);
        }
        that.router.navigate([message.data.page], message.data.pageParams);
    },

    /**
     * 此方法响应vscode 消息事件，更新token
     * @param data vscode 发送的事件参数
     */
    updateToken(data: any) {
        ((top as any).webviewSession || {}).setItem('token', data.data.token);
    },
    ...
    /**
     * 此方法响应vscode消息处理,读取IDE版本
     * @param message 事件参数
     */
     readVersionConfig(message: any) {
        if (that.callbacks[message.cbid]) {
            that.callbacks[message.cbid](message.data);
            delete that.callbacks[message.cbid];
        }
    ...
    /**
     * 此方法响应vscode消息处理，intellij右键上传文件
     * @param message 事件参数
     */
    rightClickUploadPortingFile(message: any) {
        if (that.callbacks[message.cbid]) {
            that.callbacks[message.cbid](message.data);
            delete that.callbacks[message.cbid];
        }
 ...
 * 官网地址
 */
export const enum OFFICIAL_WEBSITE {
    PORT = ''
}
