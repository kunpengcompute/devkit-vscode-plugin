export const I18N_US = {
  home_top_growth:
    'Visit the Kunpeng community to get skills for new developer growth.',
  ...
  common_term_valition_input:
    'The value cannot contain Chinese characters, spaces, and the ' +
  ...
  common_term_valition_rule2:
    'The new password cannot be the old password in reverse order.',
  common_term_valition_rule4:
    'The compilation command must start with make, cmake, or go, for example, make xxx.',
  common_term_valition_rule5:
    'The compile command parameter cannot contain Chinese \
    and the following special characters: < > | # ; & ` %.',
  common_term_valition_rule6:
    'The compilation command must start with make or cmake, for example, make xxx.',
  ...
  common_term_no: 'No',
  common_term_leave_tip2:
    'If you leave this page, the data on the current page will be lost.',
  plugins_tuning_message: {
    workInfo:
      'Insufficient workspace of the Profiler Advisor. Please delete some reports. ' +
      'Total workspace: {0} GB. Free workspace: {1} GB. Recommended free workspace: > 20% ({2} GB).',
    workWarn:
      'The free workspace of the Profiler Advisor is less than 1 GB. The tool has stopped writing data. ' +
      'Please delete some reports immediately to release the workspace. ' +
      'Total workspace: {0} GB. Free workspace: {1} GB. Recommended free workspace: > 20% ({2} GB).',
    diskInfo:
      'Insufficient drive space of the Profiler Advisor. Please release the drive space. ' +
      'Total drive space: {0} GB. Free drive space: {1} GB. Recommended free drive space: > 20% ({2} GB).',
    diskWarn:
      'The free drive space of the Profiler Advisor is less than 1 GB. The tool has stopped writing data. ' +
      'Please release the drive space immediately. ' +
      'Total drive space: {0} GB. Free drive space: {1} GB. Recommended free drive space: > 20% ({2} GB).',
    workDiskError:
      'The remaining drive space is less than 1 GB. \
          Please release the drive space and perform the next step.',
  },

  plugins_tuning_tip_softPackInstallPath:
    'Enter the full path of the installed software, \
  ...
  plugins_tuning_message_beforeInstallDsc1:
    '1. If the yum/apt/zypper source of the OS is correctly configured \
  ...
  plugins_tuning_message_beforeInstallDsc2:
    'Configuring the yum/apt/zypper Source for the OS',
  plugins_tuning_message_beforeInstallDsc3:
    '" in the Kunpeng Porting Advisor User Guide.',
  plugins_tuning_message_beforeInstallDsc4:
    '2. This tool identifies your system status based on the \
  ...
  plugins_tuning_message_beforeInstallOption:
  ...
  plugins_cloudied_tuning_label_config:
    'Configure the remote server address for installing porting advisor.',
  plugins_tuning_button_save: 'Save',
  plugins_tuning_button_modi: 'Modify',
  plugins_tuning_button_cancel: 'Cancel',
  plugins_common_message_installDt1:
    'The install will complete automatically if the remote server is running a ',
  plugins_common_message_installDt2: 'compatible',
  plugins_common_message_installDt3:
    ' operating system and can access the Internet.',
  plugins_common_message_upgradeDt1:
    'The upgrade will complete automatically if the remote server is running a ',
  plugins_common_message_upgradeDt2: 'compatible',
  plugins_common_message_upgradeDt3:
    ' operating system and can access the Internet.',
  plugins_common_message_uninstallDt:
    'The uninstall will complete automatically \
  ...
  plugins_tuning_message_portError:
    'Enter a correct port number range.(1024-65535)',
  plugins_tuning_message_bothError: 'Enter correct Ip and port.',
  ...
  plugins_tuning_message_install_failed_1:
    'Rectify the fault based on the installation \
  ...
  plugins_tuning_message_comm:
    'Visit the Kunpeng community to learn about Kunpeng DevKit',
  ...
  plugins_common_tips_start_deploy:
    'Click [Install] to start installing Hyper Tuner',
  plugins_common_tips_start_uninstall:
    'Click [Uninstall] to start uninstalling Hyper Tuner',
  plugins_common_tips_start_upgrade:
    'Click [Upgrade] to start upgrading Hyper Tuner',
  plugins_common_tips_connFail:
    'The SSH connection check failed. Check whether the user name, password, \
      or private key is correct. A large number of retries also causes the check failure.',
  plugins_common_tips_connTimeout:
    'The SSH connection check failed. Check whether the ip or the port is correct.',
  plugins_common_tips_figerFail:
    'Failed to set up the connection because the host fingerprint verification failed.',
  ...
  plugins_common_message_passphrase:
    'Enter the passphrase of the private key file.',
  plugins_common_message_sshkeyFail:
    'Import the correct id_rsa private key file.',
  plugins_common_message_passphraseFail: 'Enter the correct passphrase',
  plugins_common_message_sshkeyExceedMaxSize:
    'The private key file size exceeds 10 MB.',
  ...
  plugins_common_message_fileName:
    'The file name can contain only uppercase and lowercase letters, digits, \
  ...
  plugins_common_message_fileName_length:
    'The maximum length of a single filename is 255 characters.',
  plugins_common_message_command_length:
    'The command input box can contain no more than 1024 characters.',
  common_term_valition_realpath:
    'Enter a proper absolute path, for example, /home/pathname/',
  common_term_valition_path_length:
    'The path name can contain a maximum of 1024 characters.',
  plugins_common_tips_sshError: 'SSH connection exception. Please try again.',
  plugins_common_message_figerLose:
    "The authenticity of host {0} can't be established. The fingerprint is SHA256:{1}. \
  Are you sure you want to continue connecting?",

  plugins_common_tips_uploadError:
    'Failed to upload the script file to your server.',

  plugins_common_title_ipSelect:
    'The tool is about to send a login request to IP address {0}.\
  ...
  plugins_common_tips_ipSSH:
    'Use the IP address {0} entered for SSH connection',
  plugins_common_tips_ipExtra: 'Set IP address',

  plugins_tuning_weakPassword: {
    pwd_rule:
      'The weak password must contain at least two types of the following characters: \
  ...
  period: '.',
  semicolon: '; ',
  plugins_tuning_apply_free_env_info:
    'You can also apply for a free trial environment.',
  plugins_tuning_apply_free_env_link: 'Click here',
  plugins_tuning_free_trial_remote_environment:
    'Apply For Trial Remote Laboratory',
  plugins_common_title_config: 'Configure Remote Server',
  plugins_common_label_config:
    'Configure the remote server address for Hyper Tuner. \
    If you have not deployed the tool on the server,',
  plugins_common_button_install: 'click here to deploy',
  plugins_common_apply_free_env_info:
    'You can also apply for a free trial environment.',
  ...
  plugins_common_message_beforeInstallDsc4:
    '1. This tool identifies your system status based on the information \
    you entered \
  ...
  plugins_common_message_beforeInstallDsc5:
    '2. During the use of this tool, you may need to download and \
  ...
  plugins_tuning_message_beforeInstallDsc6:
    '3. During the use of this tool, the tool may download \
  ...
  plugins_common_message_networkErrorYunTip:
    '2. If you use a HUAWEI cloud server, ensure that the server address is \
  ...
  plugins_common_message_networkErrorYunTip_deployScenario:
    '2. If you use a HUAWEI cloud server, \
   ensure that the server address is \
  a public IP address of HUAWEI CLOUD. You cannot access the server with a private IP address. \
  If the peer server is not a HUAWEI cloud server, check for the following issues:',
  plugins_common_message_connIssue1:
    '1. The IP address entered does not exist.',
  plugins_common_message_connIssue2:
    '2. The DevKit application is not installed on the server, \
     or the server port {0} is incorrect.',
  plugins_common_message_connIssue2_deployScenario:
    '2) The server has SSH disabled.',
  plugins_common_message_connIssue3:
    '3. The network cables are not properly connected.',
  plugins_common_message_connIssue4:
    '4. Network policies, such as interception rules, are used.',
  plugins_common_message_connIssue5:
    '5. Check the network status and network configurations.',
  plugins_common_message_serverErrorTip:
    '3.  Log in to the server OS and check the host or \
  ...
  plugins_common_message_serverErrorResult2_1:
    '- If no, rectify the fault by following the \
  ...
  plugins_common_label_guide:
    'Please configure the remote server after completing the deployment of Kunpeng performance analysis tool. \
  ...
  plugins_common_guide: {
    ...
    install_serve_info:
      'Deploy Kunpeng performance analysis tool to remote server.',
    install_now: 'Start Deployment',
    config_serve: 'Configure Remote Server',
    config_serve_info:
      'Connect to the remote server where Kunpeng performance analysis tool has been deployed.',
  ...
  plugins_common_cut_server:
    'Do I need to close the page and switch server configurations? Click OK to exit the current server?',
  ...
  plugins_common_tips_checkConn_noroot:
    'You are using a common user account {0}. Check that the following conditions are met: \r\n\
  ...
  plugins_tuning_message_save_config_success:
    'Configure remote server successfully!',
  plugins_tuning_title_save_config: 'Save Remote Server Config',
  plugins_tuning_save_config_modify:
    'Save current modification? Click OK to exit the current user.',
  ...
  plugins_common_message_sshAlgError:
    'Connection detection failed. The algorithm on the client side does not match that on the \
     server side. For details about how to configure a security algorithm, see FAQ.',
  plugins_common_message_sshClientCheck:
    'The SSH client is not installed on the device. Obtain and install the SSH client.',
