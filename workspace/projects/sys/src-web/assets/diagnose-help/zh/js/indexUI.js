...
function assembleNavi(childNavigation, jsInfo) {
    ...
    //Clear the mergeProject attribute in the subproject navigation tree and change the relative path of each page of the subproject to ref + local.
...
function init() {
    ...
    addListener();
    initTips();
    console.log("language:" + language + "  browser:" + browser);
}
...
function getTopic(url, isHighlight) {
    ...
    $("#iframeContent").on("load", {highlight: isHighlight}, function (e) {
        ...
        var aText = $(this).contents().find('a');
        ...
        var selectSearchNode = function(url) {
            ...
            if (currentSelectNode) {
                ...
                $(currentSelectNode).siblings().removeClass('selected');
...
// This step is required because the system loading is supported. The sub-nav_json.js file is read. The file also contains the topLanguage attribute. Do not overwrite this attribute. You can assign a value to the sub-nav_json.js file again.
function initArgs() {
...
// This step is required because the RAT loading is supported. The nav_json.js sub-file is read. The file also contains the topMergedProjects attribute. Do not overwrite this attribute. Set this attribute again.
