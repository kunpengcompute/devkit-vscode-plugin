...
export class LiquidFillGaugeComponent implements OnInit, AfterViewInit, OnChanges {
  @Input() width: string;
  @Input() value = 0;
  @Input() gaugeColor: any;  // 球的颜色【与值无关】

  @ViewChild('fillgauge') fillgauge: any;

  public gauge: any;
  public changes: any; // 保存gauge生成之前的changes值
  ...
  public liquidfillgauge(element: any, value: any, gaugeColor: any, settings: any) {
    ...
    const defaultConfig: any = {
      ...
      // Styles
      circleThickness: 0.05, // 外圆厚度占半径的百分比
      circleFillGap: 0.05, // 外圆和内部的波形园之间的间隙大小，以外圆半径的百分比表示。
      backgroundColor: null, // 背景颜色
      width: 0, // 如果插件没有正确检测到宽度和高度，您可能希望设置宽度和高度
      height: 0,

      // Waves
      waveHeight: 0.05, // 波高占波形园的百分比
      waveCount: 1, // 波的个数
      waveOffset: 0, // 多层波时的偏移量。0=无偏移。1 = 1个满波的偏移。

      // Animations
      waveRise: true, // 控制波是否应从0升至其全高，或从其全高开始
      waveRiseTime: 1000, // 波从0上升到最终高度的时间（以毫秒为单位）
      waveRiseAtStart: true, // 如果设置为false，且waveRise为true，则仅禁用初始动画
      waveAnimate: true, // 控制波是滚动还是静止
      waveAnimateTime: 18000, // 全波进入波圈的时间（毫秒）.
      // 在低填充百分比和高填充百分比下控制波形大小缩放。
      // 当为真时，波浪高度在50%填充时达到最大值，在0%和100%填充时达到最小值。
      // 这有助于防止波使波圈在接近其最小或最大填充时显得完全满或空.
      waveHeightScaling: true,
      valueCountUp: true, // 如果为true，则显示的值在加载和更新时从0开始计数到最终值。如果为false，则显示最终值.
      valueCountUpAtStart: true, // 如果设置为false，且valueCountUp为true，则仅禁用初始动画
    };
    const idGenerator = (() => {
      let count = 0;
      return (prefix: any) => {
    ...
    const fillPercent = Math.max(config.get('minValue'), Math.min(config.get('maxValue'), value))
      / config.get('maxValue');
    ...
    const clipArea: any = d3.area()
      ...
      .y0((d: any) =>
        waveScaleY(
          Math.sin(Math.PI * 2 * config.get('waveOffset') * -1
            + Math.PI * 2 * (1 - config.get('waveCount')) + d.y * 2 * Math.PI)))
