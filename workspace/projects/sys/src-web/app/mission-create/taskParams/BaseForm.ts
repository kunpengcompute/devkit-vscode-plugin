...
class BaseForm {
  ...
  public generateFormGroup() {
    ...
    function mapItems(items: any) {
      const selectedItems = items.filter((item: any) => item.checked).map((item: any) => item.value);
    ...
    Object.keys(this.form).forEach(key => {
      ...
      if (formItem.type === 'checkbox') {
        const checkboxGroup = new FormArray(formItem.list.map((option: any) => new FormGroup({
        ...
        const hiddenControl = new FormControl(
          mapItems(checkboxGroup.value), formItem.required ? Validators.required : undefined);
    ...
    Object.keys(this.form).forEach(key => {
      ...
      if (['select', 'radio'].includes(formItem.type)) {
        this.formGroup.get(key).valueChanges.subscribe((val: any) => {
  ...
  // 修改表单值 --> 修改显示列表
  public setDisplayedElementListWhenChange({ key, formItem, value }: any) {
    // 计算列表【迭代children计算出总的列表】
    const calcList = (tar: any) => {
      const list: any = [];
      const addList = (param: any) => {
  ...
  // 初始化默认值
  public initDefaultValue({ list }: any) {
    list.forEach((key: any) => {
      const el = this.form[key];

      if (!el.hasSetDefaultValue) {
        if (['select', 'radio'].includes(el.type)) {
          const option = el.list.find((opt: any) => opt.checked);
  ...
  public setDisplayedElementList({ operate, list }: {
    ...
    if (operate === 'add') {
      list.forEach(item => {
        ...
        if (!this.form[item].hasSetDefaultValue) {
          this.initDefaultValue({ list: [item] });
        }
      });
  ...
  public getFormControlValue({ formKey, formKeyList, type, formEl }: {
    ...
    if (type === 'valueArray') {
      if (formKey) {
        ...
        return formKeyList.map(key => calcValue(key));
      }
  ...
  public valuesToParams({ values, removeNulls, nullValue = ['', undefined, null] }: {
    ...
    const params = {};
    const setParams = (currentParams: any, interfaces: any) => {
      Object.keys(interfaces).forEach(key => {
        ...
        if (['string', 'number'].includes(typeof item)) { // string、number类型: 表单上没有但是后端需要的参数
          ...
          if (item.formKey) { // formKey 表示对应表单的某个参数
            if (Object.prototype.hasOwnProperty.call(values, item.formKey)) {
            ...
            const value = item.interFaceMapping({
              operate: 'get',
              formControlValueList: item.formKeyList.map((formKey: any) => values[formKey]),
              params
            });
  ...
  public paramsToValues({ params }: {
    ...
    const values: any = {};
    const setParams = (currentValues: any, paramInterfaces: any) => {
      Object.keys(paramInterfaces).forEach(key => {
        ...
        if (['string', 'number'].includes(typeof item)) { // string、number类型表单上没有但是后端需要的参数，不需要处理
          ...
          if (item.formKey) { // formKey 表示对应表单的某个参数
            ...
            item.formKeyList.forEach((formKey: any, index: any) => {
  ...
  // 设置表单元素参数
  // -- TODO --数据回显优化，undefined时显示 --,可以再对接口映射进行详细分类，分为formKey、getValue、setValue、setText
  public setFormControlValue({ formEl, type, formKey, value, i18n }: {
    ...
    if (['select', 'radio'].includes(formItem.type)) {
      let option;
      if (value) {
        option = formItem.list.find((item: any) => item.value === value);
      } else {
        option = formItem.list.find((item: any) => item.checked);
      }
      ...
      formEl.formGroup.controls[formKey].controls.forEach((formGroup: any) => {
        formGroup.controls.checked.setValue(Array.isArray(value) && value.includes(formGroup.controls.value.value));
      });

      if (type === 'text') {
        const list: any = [];
        formEl.formGroup.get(formKey).value.forEach((option: any) => {
          if (option.checked) {
