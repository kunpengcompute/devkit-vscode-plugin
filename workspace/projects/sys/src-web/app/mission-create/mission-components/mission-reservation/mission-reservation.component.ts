...
export class MissionReservationComponent implements OnInit, OnDestroy, OnChanges {
  constructor(public i18nService: I18nService, private Axios: AxiosService) {
  ...
  public getCurrentServerTime() {
    this.Axios.axios.get('/schedule-tasks/current-time/').then((res: any) => {
  ...
  public firstInitDateAndTime() {
    ...
    this.onceTime = this.nowTime.nowYear + '-' + this.nowTime.nowMonth + '-' + (this.nowTime.nowDay + 1)
      + ' ' + this.nowTime.nowHour + ':' + this.nowTime.nowMinutes + ':' + this.nowTime.nowSecond;
    this.durationTime = this.nowTime.nowYear + '-' + this.nowTime.nowMonth + '-'
  ...
  // 设置单次采集最小最大日期
  public setOnceTime() {
    this.minOnce = new Date(this.nowTime.nowYear, this.nowTime.nowMonth - 1, this.nowTime.nowDay,
      this.nowTime.nowHour, this.nowTime.nowMinutes, this.nowTime.nowSecond);
  ...
  // 关闭预约组件,清理数据
  public clear() {
    this.switchState = false; // 关闭预约组件
    this.selected = 1;
    this.clearServerInterval();
    this.isEdit = false; // 用于修改时将开关按钮设置为disable
    this.isDisable = false; // 用于重启时将开关按钮设置为disable
    this.nowTime = {
      nowFullTime: '',
      nowYear: 0,
      nowMonth: 0,
