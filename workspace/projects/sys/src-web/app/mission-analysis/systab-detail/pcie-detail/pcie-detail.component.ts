...
import {
  CommonTableData,
  CommonTreeNode,
} from 'sys/src-com/app/shared/domain';
...
export class PcieDetailComponent implements OnInit {
  ...
  // 进程解绑状态表格数据
  public processTableData: CommonTableData = {
    srcData: {
      data: [],
      state: {
        searched: false,
    ...
    columnsTree: this.getProcessColumns(),
  };

  public displayedCpu: Array<TiTableRowData> = []; // 表示表格实际呈现的数据（开发者默认设置为[]即可）
  ...
  public currentPagePro = 1;
  public totalNumberPro: number;
  public pageSizePro: { options: Array<number>; size: number } = {
    options: [10, 20, 50, 100],
    size: 10,
  };
  ...
  ngOnInit(): void {
    ...
    this.hardInterruptOptions = [
      {
        label: this.i18n.pcieDetailsinfo.interruptsView,
        value: 'interruptsView',
      },
      { label: this.i18n.pcieDetailsinfo.cpuView, value: 'cpuView' },
    ];
  ...
  getNodeData(node: Tree) {
    ...
    this.nodeConfigInfoKeys = Object.keys(
      node?.nodeData?.node_config_info || {}
    );
  }
  // 获取数据
  getPcieDetails() {
    this.Axios.axios
      .get(
        `/tasks/${this.taskId}/sys-performance/summary/?queryTarget=PCIe&queryType=summary&nodeId=${this.nodeId}`
      )
      .then((res: any) => {
        if (res.data.numa_common_config_info.cpu_usage_info) {
          ...
          this.singleIrqCPUData =
            res.data.numa_common_config_info.eth_cpu_irq_info;
          this.softwareInterData =
            res.data.numa_common_config_info.softirq_info;
          ...
          // 硬中断绑核状态 中断编号视图
          this.irqBindingInfo = this.changeObjToArry(
            res.data.numa_common_config_info.irq_affinity_mask
          );
          this.srcDataIrq.data = this.irqBindingInfo;
          this.totalNumberIrq = this.irqBindingInfo.length;

          // 硬中断绑核状态 CPU核视图
          this.irqCpuInfo = this.changeObjToArry(
            res.data.numa_common_config_info.cpu_irq_info
          );
          this.srcDataIrqCpu.data = this.irqCpuInfo;
          this.totalNumberIrqCpu = this.irqCpuInfo.length;

          // XPS/RPS绑核状态信息 中断编号视图
          this.xpsBindingInfo = this.changeObjToArry(
            res.data.numa_common_config_info.descriptor_affinity_mask
          );
          this.srcDataXps.data = this.xpsBindingInfo;
          this.totalNumberXps = this.xpsBindingInfo.length;

          // XPS/RPS绑核状态信息 CPU核视图
          this.xpsCpuInfo = this.changeObjToArry(
            res.data.numa_common_config_info.cpu_descriptor_info
          );
        ...
        // Top10 进程绑核状态 进程视图
        this.processCoreBinding = this.changeObjToArry(
          res.data.numa_common_config_info.process_affinity_mask
        );
        this.processTableData.srcData.data = this.processCoreBinding;
        this.processTableData = { ...this.processTableData };
        this.totalNumberPro = this.processCoreBinding.length;

        // Top10 进程绑核状态 CPU核视图
        this.processCpu = this.changeObjToArry(
          res.data.numa_common_config_info.cpu_process_info
        );
  ...
  public getNodeDetailsItemObjKeys(obj: any) {
    const nodeDetail = Object.keys(obj).filter(
      (item) =>
  ...
  private getProcessColumns(): CommonTreeNode[] {
    const columns = [
      {
        key: 'key',
        label: I18n.pcieDetailsinfo.processTitle1,
        width: '33%',
        checked: true,
        searchKey: 'key',
        canClick: true
      ...
      {
        key: 'cmdline',
        label: I18n.pcieDetailsinfo.processTitle2,
        width: '33%',
        checked: true,
        searchKey: 'cmdline',
      },
      {
        key: 'affinity_mask',
        label: I18n.pcieDetailsinfo.processTitle3,
        width: '34%',
        checked: true,
      },
    ];
  ...
  private openViewDetailMask(modal: TemplateRef<any>) {
    this.tiModal.open(modal, {
      modalClass: 'view-detail-mask-modal',
    });
  }
}
