import {
    ...
    OnDestroy, ChangeDetectorRef, AfterViewChecked
} from '@angular/core';
...
export class ExtrapositionAxisComponent implements AfterViewInit, OnDestroy, AfterViewChecked {
    ...
    public attachVernierEvent(vLeftSlection: JQuery, vRightSelection: JQuery) {
        ...
        // 处理mousemove事件的数据 FIXME 在这里 mousemove 事件会被重复的绑定
    ...
    public attachHotspotEvent(hotSelection: JQuery): () => void {
        ...
        const mousemoveHandler = (evt: any) => {
            ...
            this.currentHotspotDomain = [this.originalAxisScale.invert(nowHotRange[0]),
    ...
    private renderHotspot(domain: any) {
    ...
    private renderHotspotByRange(range: any) {
    ...
    private setTickInfoList(option: any) {
        ...
        const tickStartTextWidth = this.calculateTextWidth(tickStart.value, { fontSize: this.tickFontSize });
        const tickEndTextWidth = this.calculateTextWidth(tickEnd.value, { fontSize: this.tickFontSize });
    ...
    private calcOptimalTickList(domain: [number, number], limit: [number, number]): number[] {
        ...
        function calcCompleteMore(val: number): number[] {
            ...
            const num = Math.round(val * BOUNDARY);
