import { Component, Input, Output, ElementRef, HostListener, AfterViewInit,
   AfterViewChecked, SecurityContext, EventEmitter } from '@angular/core';
...
export class IcicleSummaryComponent implements AfterViewInit, AfterViewChecked {
  ...
  constructor(
    ...
    // 生成随机数
    this.chartID = 'summury_icicle_chart_' + axios.generateConversationId(15); // 获取一个唯一的ID值
    this.tooltipID = 'summury_icicle_chart_tip_' + axios.generateConversationId(15);
  }
  ...
  public render(ctx: any, data: any) {
    ...
    const clipPathId = ctx.axios.generateConversationId(15);
    const polygonStrokeWidth = 2;
    const rectBetweenGap = 3;
    const tooltipWidth = 326; // 这个值是样式中写死的，单位为px
    const rectTextLimitWidth = 16;
    const shapeChangeDuration = 600; // 单位毫秒
    const defaultFontSize = '14px'; // 字体大小
    ...
    function handleCellClick(event: MouseEvent, p: any) {
      ...
      const t = cell.transition().duration(shapeChangeDuration)
        .on('start', () => {
      ...
      text.call(setCellText);
    }

    function setCellText(textSelection: any) {
      const ellipsisTextWidth = getSvgTextWidth('...');
      textSelection.each((d: any) => {
        const sumWidth: number = getRectWidth(d);
        let nameWidth: number = getSvgTextWidth(d.data.name);
        const imageWidth: number = d.data.maxValue ? 19 : 0;
      ...
      const trans = d3.transition().duration(shapeChangeDuration);
      ...
      maxValue.append('text')
        .text((d: any) => {
          return (d.x1 - d.x0) > rectTextLimitWidth
            ? getOmitText(d.data.maxValue + '%', d.renderTextInfo.maxValueWidth, d.actualTextInfo.maxValueWidth)
            : undefined;
        });

      function getOmitText(texts: string, limitWidth: number, actualWidth: number) {
        if (ellipsisTextWidth > limitWidth) {
          return '';
        } else if (Math.abs(limitWidth - actualWidth) < 0.00001 || limitWidth > actualWidth) {
          return texts;
        } else {
          return recursive(0, texts.length - 1);
        }
        ...
        function recursive(startIndex: number, endIndex: number): any {
          ...
          if (endIndex - startIndex === 1) {
            const optimalText = texts.substring(0, startIndex + 1) + '...';
          ...
          const omitText = texts.substring(0, pos + 1) + '...';
      ...
      function getSvgTextWidth(texts: any, font = { fontSize: defaultFontSize, fontFamily: 'inherit' }) {
    ...
    function getRectFill(d: any) {
      if (d.data.name.toLowerCase() === 'retiring') {
        return '#C3F8CC';
      }
      return '#D4E0FF';
    }

    function getRectHeight(d: any): number {
    ...
    function handleMouseenter(event: MouseEvent, d: any): void {
      ...
      rect.attr('fill', (dd: any) => {
        return dd.over ? '#7FA0FC' : getRectFill(dd);
      });

      // 设置字体的颜色
      text.attr('fill', (dd: any) => {
        return dd.over ? 'rgba(255,255,255,1)' : 'rgba(43,49,63,1)';
      });

      // 初始化提示框的值赋值
      let content: string;
      if (d.data.suggestions == null) {
        content = `<p>${ctx.i18n.micarch.eventName}: ${
          ctx.domSanitizer.sanitize(SecurityContext.HTML, d.data.name)}<p/>
              <p>${ctx.i18n.micarch.percentage}: ${
                ctx.domSanitizer.sanitize(SecurityContext.HTML, d.data.proportion)}</P>`;
      } else {
        const desc = d.data.suggestions.desc == null ? '' : ctx.i18n.micarch.suggestions.des + d.data.suggestions.desc;
        ...
        const point = `
          <p>${ctx.domSanitizer.sanitize(SecurityContext.HTML, desc)}</p>
          <p>${ctx.domSanitizer.sanitize(SecurityContext.HTML, cause)}</p>
          <p>${ctx.domSanitizer.sanitize(SecurityContext.HTML, tips)}</p>
        `;
        content = `
          <p>${ctx.i18n.micarch.eventName}: ${ctx.domSanitizer.sanitize(SecurityContext.HTML, d.data.name)}</p>
          <p>${ctx.i18n.micarch.percentage}: ${ctx.domSanitizer.sanitize(SecurityContext.HTML, d.data.proportion)}</p>
          ${ctx.domSanitizer.sanitize(SecurityContext.HTML, point)}
        `;
      }
      ...
      tooltip.stop()
        ...
        .html(ctx.domSanitizer.sanitize(SecurityContext.HTML, content));
    }
    ...
    function getLinePathByAxis(d: any): [number, number][] {
      let p = d;
    ...
    function getPartition(roots: HierarchyNode<any>): any {
      roots.sum(d => d.value);
      roots.count();
      const p = d3.partition()
        .size([width, (roots.height + 1) * (height - (padding.top + padding.bottom)) / 6])
        (roots);
    ...
    function reasonableData(val: any) {
      const datas = deepClone(val);

      function recursive(node: any, pValue: any) {
      ...
      recursive(datas, datas.proportion);
      if (datas.name !== 'HPC top-down') {
        findMax(datas);
      }
      return datas;
    }
