...
import {
  TimeLineComponent
} from 'projects/sys/src-com/app/shared/components/time-line/time-line.component';
...
export class DdrSummuryComponent implements OnInit {
  ...
  private async getSysInfo(): Promise<RespSysInfo> {
    const resp = await this.axiosService.axios.get(
      `/tasks/${this.taskid}/mem-access-analysis/?node-id=${this.nodeid}&query-type=summary&query-target=sysinfo`,
  ...
  private async getData(queryTarget: string) {
    ...
    const resp = await this.axiosService.axios.get(`/tasks/${this.taskid}/mem-access-analysis/`, {
      params,
      headers: { showLoading: false }
    });
  ...
  private async initTimeLine() {
    ...
    const resp = await this.axiosService.axios.get(`/tasks/${this.taskid}/common/configuration/`, {
      params,
      headers: { showLoading: false }
    });
    // 单位/秒
