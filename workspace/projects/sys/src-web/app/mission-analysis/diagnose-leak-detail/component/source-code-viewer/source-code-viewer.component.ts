...
import { TabSwitchService } from 'sys/src-web/app/mission-analysis/diagnose-leak-detail/service/tab-switch.service';
...
export class SourceCodeViewerComponent implements OnInit, OnChanges, OnDestroy, AfterViewChecked, AfterViewInit {
    ...
    ngOnChanges(changes: SimpleChanges): void {
                    ...
                    decorations.push({
                        ...
                        options: {
                            ...
                            glyphMarginHoverMessage: {
                                value: `${this.data.functionSourceInfo.selfline[+line].join?.(' ')}`
                            }
                        }
                    });
        ...
        if (changes.memLeakType) {
            ...
            const previousIsLeak = [
                MemLeakType.leakCount,
                MemLeakType.leakSize,
            ].includes(changes.memLeakType.previousValue);
            const currentIsLeak = [
                MemLeakType.leakCount,
                MemLeakType.leakSize,
            ].includes(changes.memLeakType.currentValue);
