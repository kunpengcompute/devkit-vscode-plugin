import {
  AfterViewInit, Component, ElementRef, Input, EventEmitter,
  Renderer2, ViewChild, AfterViewChecked, HostListener, Output,
...
export class NormalizeBlockChartComponent implements AfterViewInit, AfterViewChecked {
  ...
  ngAfterViewChecked(): void {
    if (this.prevClientWidth === 0 && this.containerSelection.width() > 0) { // 渲染的必要条件: 视图突变
      if (this.doneRender) { return; }
      this.currentNode = this.load(this.d3Container.nativeElement, this.rawData, null);
  ...
  private render(
    ...
    // FIXME
    color.All = 'rgba(173,78,0,0.1)';
    const self: NormalizeBlockChartComponent = this;
    ...
    const series = seriesBuilder(data)
      .map(d =>
        (d.forEach((v: any) => v.key = d.key), d)
      );
    ...
    svg.append('g')
      ...
      .on('mouseenter', function f(event, d: any) {
        ...
        self.blockMouseenter.emit(poiBlock);
        d3.select(this)
          .attr('fill', '#AD4E00');
      })
      .on('mouseleave', function f(event, d: any) {
        self.blockMouseleave.emit();
        d3.select(this)
   ...
   * {
   *   data: [
   *     {time: 0, 0kb-8kb: {value: 1, percent: 10,},
   * 8kb-16kb: {value: 7, percent: 70,}, 16kb-32kb: {value: 1, percent: 10,},},
   *     {time: 1, 0kb-8kb: {value: 3, percent: 30,},
   * 8kb-16kb: {value: 3, percent: 30,}, 16kb-32kb: {value: 7, percent: 70,},},
   *     {time: 2, 0kb-8kb: {value: 7, percent: 70,},
   * 8kb-16kb: {value: 1, percent: 10,}, 16kb-32kb: {value: 3, percent: 30,},},
   *   ],
   *   columns: [
   *     'time',
   *     '0kb-8kb',
  ...
  private transform(rawData: NormalizeBlockData): { data: ChartData, columns: string[] } {
    // FIXME 填充空白
    rawData.data.All = new Array(rawData.time.length).toString().split(',').map(() => 0.000001);
