...
export class FunctionInfoComponent implements OnInit, AfterViewInit {
  ...
  set functionDetails(val: IFunctionDetails) {
    ...
    if (val != null) {
      ...
      this.sourceData.noData = val.sourceCode.message == null
        ...
        : this.i18n.common_term_task_tab_function_source_file_path;
    }
  }
  get functionDetails() {
   ...
   */
  ngOnInit(): void {
    this.boxID = this.Axios.generateConversationId(16);
    this.demandLoadInfo = {
  ...
  public formatTreeData(data: any) {
    ...
    data.forEach((item: any, index: any) => {
      ...
      if (item.children) {
        ...
        item.trackId = this.Axios.generateConversationId(8);
      }
    });
    this.assemblerDataIterator = Util.sliceArray(data, 10, 15);
