...
export class AnsiUpService {
  ...
  private get_next_packet(): TextPacket|any {
    ...
    if (pos === 0) {
      ...
      if (nextChar === ']') {
        ...
        {
          const mat = this.oscSt.exec(this.buffer);
  ...
  private transform_to_html(fragment: TextWithAttr, withTag: boolean): string {
    ...
    return `<span${styleString}${classString}>${this.domSanitizer.sanitize(SecurityContext.HTML, txt)}</span>`;
  }
