...
export class PopDirective implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    if (this.popContent) {
      ...
      document.body.addEventListener('scroll', this.onClick.bind(this), true);
    }
  }
  ...
  public show() {
    ...
    const outWidth = offsetX + popRect.width - window.innerWidth;
    if (outWidth > 0) {
      offsetX -= outWidth;
    }
    const outHeight = offsetY + popRect.height - window.innerHeight;
    if (outHeight > 0) {
      offsetY -= outHeight;
    }
  ...
  ngOnDestroy() {
    ...
    document.body.removeEventListener('scroll', this.onClick.bind(this), true);
  }
}
