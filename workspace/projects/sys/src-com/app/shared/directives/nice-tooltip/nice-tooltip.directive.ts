import {
  ...
  AfterViewInit, TemplateRef, EmbeddedViewRef,
  ApplicationRef,
} from '@angular/core';
...
export class NiceTooltipDirective implements AfterViewInit {
  ...
  public resetValue() {
    ...
    const template = this.niceTooltipInfo.html;
    const context = this.niceTooltipInfo.context;

    if (template instanceof TemplateRef) {
      const embeddedView: EmbeddedViewRef<any> = template.createEmbeddedView({ context });
      ...
      tooltipSelection.empty();
      tooltipSelection.append($(template));
    }
  }
