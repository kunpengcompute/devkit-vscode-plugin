...
import { RespIndicatorThreshold } from '../domain/resp-indicator-threshold.type';

@Component({
  selector: 'app-process-cpu-target',
  templateUrl: './process-cpu-target.component.html',
  styleUrls: ['./process-cpu-target.component.scss']
})
export class ProcessCpuTargetComponent implements OnInit, OnDestroy {

  @Input() viewCpuTarget: ProcessCpuTarget;

  @ViewChild('thresholdPop') thresholdPop: PopDirective;
  ...
  /** cpu指标下拉框选中值 */
  public cpuTargetSelected: ProcessCpuTarget;
  /** 每个指标对应的阈值范围 */
    ...
    this.subTopologyTree = this.taskDetailMessageService.getMessege({
      next: (message: TopologyTreeMessageDetail<ViewProcessThreadData>) => {
        if (message.type === TopologyTreeMessageType.viewTopProceeThread) {
          this.cpuTargetSelected = message.data.viewCpuTarget;
          this.cpuTargetSelectedChange();
        }
      }
    });
  }
  ...
  private async initTargetThreshold() {
    ...
    resp.forEach(item => {
      ...
      this.cpuTargetOptions.push({
        label: '%' + item.metric,
        value: item.metric
      });
    });
    this.cpuTargetSelected = this.viewCpuTarget;
  ...
  /**
   * 每次打开气泡弹窗时传入一个新的上下文，使弹窗里面不确定的修改不会影响到外部
   */
