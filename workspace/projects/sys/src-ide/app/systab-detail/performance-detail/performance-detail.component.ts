import {
    Component, OnInit, Input, ElementRef, ViewChild, AfterViewInit, Renderer2, ChangeDetectorRef
} from '@angular/core';
import { AxiosService } from '../../service/axios.service';
import { I18nService } from '../../service/i18n.service';
import { MytipService } from '../../service/mytip.service';
import { LeftShowService } from '../../service/left-show.service';
import { TiTableColumns, TiTableRowData, TiModalService, TiTableSrcData } from '@cloud/tiny3';
...
export class PerformanceDetailComponent implements OnInit, AfterViewInit {
    ...
    @ViewChild('timeChartConfig', { static: false }) timeChartConfig;
    public timeChartConfigModal: any;
    @ViewChild('cpuDetail', { static: false }) cpuDetail;  // cpu组件
    @ViewChild('memoryDetail', { static: false }) memoryDetail;  // 内存组件
    @ViewChild('diskDetail', { static: false }) diskDetail;  // 磁盘IO
    @ViewChild('networkDetail', { static: false }) networkDetail;  // 网络组件
    @ViewChild('timeLineDetail', { static: false }) timeLineDetail;  // 网络组件
    @ViewChild('performanceTable', { static: false }) performanceTable;  // 网络组件
    constructor(
    ...
    public downloadWidth = 1500;
    public i18n: any;
    public isTable = false;
    public noDataInfo = '';
    public timeData = []; // 时间轴数据
    // cpu 利用率 表格
    ...
    public cpuCheckedListKey: Array<TiTableRowData> = []; // 默认选中项
    ...
    public cpuTypeOption = [];  // cpu 利用率 指标 数据
    ...
    // 磁盘IO
    public diskShow = true; // 磁盘IO是否展示
    // 磁盘IO 块设备
    public devOption = [];
    public devSelected: any;
    public deviceChange = false; // 磁盘IO块设备筛选是否修改
    // 磁盘IO 指标
    public typeOption = [];
    public typeSelected: any;
    public diskIndexChange = false; // 磁盘IO块指标筛选是否修改
    // 磁盘IO 表格
    ...
    public diskCheckedListKey: Array<TiTableRowData> = []; // 默认选中项
    ...
    public okIfaceCheckedListKey: Array<TiTableRowData> = []; // 默认选中项
    ...
    public errorCheckedListKey: Array<TiTableRowData> = []; // 默认选中项
    ...
    public filterImg = './assets/img/filterNormal.svg';
    ...
    /**
     * 弹出“筛选分析对象/指标”配置页面，初始化子组件
     */
    public chartConfig() {
        this.timeChartConfigModal = this.tiModal.open(this.timeChartConfig, {
            // 定义id防止同一页面出现多个相同弹框
            id: 'timeChartConfig',
     ...
     * cpu 自定义表格数据
     */
    public cpuTableData(data, arrData) {
        ...
        if (this.cpuNumsOptionIndex !== 3) {
            let value = 0;
            this.cpuNumsOption.forEach((element, index) => {
                if (element.id === this.cpuNumsOptionIndex.toString()) {
                    value = index;
                }
            });
            ...
            this.cpuNumSelected = this.cpuNumsOption[1];
        }

        this.cpuCheckedList = this.assignment(cpuData, arrData);
        this.cpuCheckedListKey = this.cpuCheckedList.map(item => item.id);
    ...
    public cpuTypeChange(e) {
        ...
        this.cpuTypeSelected.forEach((item, index) => {
            obj = {
                title: item.label,
        ...
        const cpuCheckedDataArr = [];
        this.cpuHeaderData.forEach((item) => {
        ...
        numaOptionkeys.forEach(item => {
            this.cpuTableOriginalData.numa_info[item].forEach((value) => {
                const cpuInfo: any = {};
                cpuInfo['name' + 0] = 'CPU' + value;
                cpuInfo['name' + 1] = 'NUMA' + item;
                cpuInfo.id = value;
                this.cpuTypeSelected.forEach((element, indexData) => {
                    const indexSelect =
                        this.cpuTableOriginalData.cpu_usage.name[
                            element.label
                        ].name.indexOf(value);
                    cpuInfo['name' + (indexData + 2)] =
                        this.cpuTableOriginalData.cpu_usage.name[
                            element.label
                        ].value[indexSelect];
                });
    ...
    public onMyChange() {
        ...
        if (length < 4) {
            this.cpuHeaderData.forEach((item, index) => {
                if (index > 1) {
                    item.disabled = false;
                }
            });
        ...
        this.cpuTitle.forEach((item, index) => {
            if (index > 1) {
                if (arr.indexOf(item.title)) {
                    item.show = true;
                }
     ...
     * 磁盘IO 表格数据
     */
    public diskTableData(data, arrData) {
        ...
        if (this.typeOptionIndex !== 3) {
            let value = 0;
            this.devOption.forEach((element, index) => {
                if (element.id === this.typeOptionIndex.toString()) {
                    value = index;
                }
            });
            ...
            this.devSelected = this.devOption[1];
        }
        this.diskCheckedList = this.assignment(diskData, arrData);
        this.diskCheckedListKey = this.diskCheckedList.map(item => item.id);
    ...
    public diskTypeChange() {
        this.diskIndexChange = true; // 磁盘IO块设备筛选是否修改
        const titleArr = [];
        let obj = {};
        if (this.typeSelected) {
            this.required(this.typeOption, this.typeSelected);
            this.typeSelected.forEach((item, index) => {
                obj = {
                    title: item.label,
        ...
        const cpuCheckedDataArr = [];
        this.diskHeaderData.forEach((item) => {
            if (item.show || item.show === undefined) {
                cpuCheckedDataArr.push(item);
            }
        });
        ...
        const diskData: any = [];
        this.diskTableOriginalData.spec.forEach((item, value) => {
            const diskInfo: any = {};
            diskInfo['name' + 0] = item;
            diskInfo.id = item;
            if (this.typeSelected) {
                this.typeSelected.forEach((element, indexData) => {
                    const indexSelect =
                        this.diskTableOriginalData.disk_usage.name[
                            element.label
                        ].name.indexOf(item);
                    diskInfo['name' + (indexData + 1)] =
                        this.diskTableOriginalData.disk_usage.name[
                            element.label
                        ].value[indexSelect];
                });
            }
            diskData.push(diskInfo);
        });
     ...
     *  网络IO 传输量 表格数据
     */
    public okIfaceSelectData(data, arrData) {
        ...
        if (this.okTypeOptionIndex !== 3) {
            let value = 0;
            this.okIfaceOption.forEach((element, index) => {
                if (element.id === this.okTypeOptionIndex.toString()) {
                    value = index;
                }
            });
            ...
            this.okIfaceSelected = this.okIfaceOption[1];
        }
        this.okIfaceCheckedList = this.assignment(okIfaceData, arrData);
        this.okIfaceCheckedListKey = this.okIfaceCheckedList.map(item => item.id);
    ...
    public okIfaceHeaderChange() {
        ...
        if (length < 4) {
            this.okIfaceHeaderData.forEach((item, index) => {
                if (index > 0) {
                    item.disabled = false;
                }
            });
        ...
        this.okIfaceTitle.forEach((item, index) => {
            if (index > 0) {
                if (arr.indexOf(item.title)) {
                    item.show = true;
                }
    ...
    public okIfaceTypeChange() {
        this.transmissionIndexChange = true; // 磁盘IO块设备筛选是否修改
        const titleArr = [];
        let obj = {};
        if (this.okTypeSelected) {
            this.required(this.okTypeOption, this.okTypeSelected);
            this.okTypeSelected.forEach((item, index) => {
                obj = {
                    title: item.label,
                    sortKey: 'name' + (index + 1),
                    show: index < 3 ? true : false,
        ...
        const cpuCheckedDataArr = [];
        this.okIfaceHeaderData.forEach((item) => {
            if (item.show || item.show === undefined) {
                cpuCheckedDataArr.push(item);
            }
        });
        ...
        this.okIfaceTableOriginalData.spec.forEach((item, value) => {
            const okIfaceInfo: any = {};
            okIfaceInfo['name' + 0] = item;
            okIfaceInfo.id = item;
            if (this.okTypeSelected) {
                this.okTypeSelected.forEach((element, indexData) => {
                    const indexSelect =
                        this.okIfaceTableOriginalData.net_info.name[
                            element.label
                        ].name.indexOf(item);
                    okIfaceInfo['name' + (indexData + 1)] =
                        this.okIfaceTableOriginalData.net_info.name[
                            element.label
                        ].value[indexSelect];
                });
            }
            okIfaceData.push(okIfaceInfo);
        });
     ...
     *  网络IO 故障 表格数据
     */
    public errorSelectData(data, arrData) {
        ...
        if (this.errorTypeOptionIndex !== 3) {
            let value = 0;
            this.errorIfaceOption.forEach((element, index) => {
                if (element.id === this.errorTypeOptionIndex.toString()) {
                    value = index;
                }
            });
            ...
            this.errorIfaceSelected = this.errorIfaceOption[1];
        }
        this.errorCheckedList = this.assignment(errorData, arrData);
        this.errorCheckedListKey = this.errorCheckedList.map(item => item.id);
    ...
    public errorHeaderChange() {
        ...
        if (length < 4) {
            this.errorHeaderData.forEach((item, index) => {
                if (index > 0) {
                    item.disabled = false;
                }
            });
        ...
        this.errorTitle.forEach((item, index) => {
            if (index > 0) {
                if (arr.indexOf(item.title)) {
                    item.show = true;
                }
    ...
    public errorTypeChange() {
        this.malfunctionIndexChange = true; // 磁盘IO块设备筛选是否修改
        const titleArr = [];
        let obj = {};
        if (this.errorTypeSelected) {
            this.required(this.errorTypeOption, this.errorTypeSelected);
            this.errorTypeSelected.forEach((item, index) => {
                obj = {
                    title: item.label,
                    sortKey: 'name' + (index + 1),
                    show: index < 3 ? true : false,
        ...
        this.errorHeaderData.forEach((item) => {
            if (item.show || item.show === undefined) {
                cpuCheckedDataArr.push(item);
            }
        });
        ...
        const errorData: any = [];
        if (this.errorTableOriginalData && this.errorTableOriginalData.spec) {
            this.errorTableOriginalData.spec.forEach((item: any) => {
                const errorInfo: any = {};
                errorInfo['name' + 0] = item;
                errorInfo.id = item;
                if (this.errorTypeSelected) {
                    this.errorTypeSelected.forEach((element: any, indexData: number) => {
                        const indexSelect =
                            this.errorTableOriginalData.net_error_info.name[
                                element.label
                            ].name.indexOf(item);
                        errorInfo['name' + (indexData + 1)] =
    ...
    public cpuTypeChange2(e) {
        if (e === 1) {
            ...
            if (this.cpuNumSelected.id === SELECTED_TOP_NUM.NUM_SELF_CONFIG) {  // 是否展示 表格筛选
                this.required(this.cpuTypeOption, this.cpuTypeSelected);
                this.cpuTableShow = true;
            } else {
                this.cpuTableShow = false;
            ...
            this.deviceChange = true; // 磁盘IO块设备筛选是否修改
            if (this.devSelected.id === SELECTED_TOP_NUM.NUM_SELF_CONFIG) {  // 是否展示 表格筛选
                this.duskTableShow = true;
                this.diskTypeChange();
            } else {
                this.duskTableShow = false;
            ...
            if (this.okIfaceSelected.id === SELECTED_TOP_NUM.NUM_SELF_CONFIG) {  // 是否展示 表格筛选
                this.okIfaceTypeChange();
                this.okIfaceTableShow = true;
            } else {
                this.okIfaceTableShow = false;
            ...
            if (this.errorIfaceSelected.id === SELECTED_TOP_NUM.NUM_SELF_CONFIG) {  // 是否展示 表格筛选
                this.errorTypeChange();
                this.errorTableShow = true;
            } else {
                this.errorTableShow = false;
    ...
    public confirmCpu(detailData: any) {
        ...
        let cpuTypeSelected = [];
        if (this.cpuNumSelected.id && this.cpuNumSelected.id === SELECTED_TOP_NUM.NUM_SELF_CONFIG) {
            this.cpuNumsOptionIndex = 2;
            this.cpuCheckedList.forEach((item) => {
                const obj = {
                    label: item.id,
                    id: item.id
                };
        ...
        } else if (this.cpuNumSelected.id && this.cpuNumSelected.id !== SELECTED_TOP_NUM.NUM_SELF_CONFIG) {
            this.cpuNumsOptionIndex = this.cpuNumSelected.id === SELECTED_TOP_NUM.NUM_TOP05 ? 0 : 1;
            const indexData = this.cpuNumSelected.id === SELECTED_TOP_NUM.NUM_TOP05 ? 4 : 9;
            this.cpuTop[this.cpuTypeSelected[0].id].name.forEach((item, index) => {
                if (index <= indexData) {
                    const obj = {
                        label: item,
                        id: item
        ...
        if (this.cpuCheckedList.length !== 0) {
            this.cpuDetail.cpuNumChange({
                data: cpuSelectData, show: this.UtilizationShow, change: this.cpuNumsChange
            });
        }
    ...
    public confirmMemory() {
        ...
        // 内存分页统计筛选方法
        this.memoryDetail.pagChange({
            data: this.pagSelected, show: this.Pagination, change: this.pageStatisticsChange
        });
    ...
    public confirmDisk() {
        // 磁盘IO 块设备筛选方法
        const diskSelectData = [];
        let typeSelected = [];
        if (this.devSelected.id === SELECTED_TOP_NUM.NUM_SELF_CONFIG) {
            this.typeOptionIndex = 2;
            this.diskCheckedList.forEach((item) => {
                const obj = {
                    label: item.id,
                    id: item.id
                };
                diskSelectData.push(obj);
            });
        } else if (this.devSelected.id !== SELECTED_TOP_NUM.NUM_SELF_CONFIG) {
            this.typeOptionIndex = this.devSelected.id === SELECTED_TOP_NUM.NUM_TOP05 ? 0 : 1;
            const indexData = this.devSelected.id === SELECTED_TOP_NUM.NUM_TOP05 ? 4 : 9;
            this.diskTop[this.typeSelected[0].id].name.forEach((item, index) => {
                if (index <= indexData) {
                    const obj = {
                        label: item,
                        id: item
    ...
    public confirmNetwork() {
        // 网络IO 接口传输数据量统计 接口筛选
        const okIfaceSelectData = [];
        let okTypeSelected = [];
        if (this.okIfaceSelected.id === SELECTED_TOP_NUM.NUM_SELF_CONFIG) {
            this.okIfaceCheckedList.forEach((item) => {
                const obj = {
                    label: item.id,
                    id: item.id
                };
        ...
        } else if (this.okIfaceSelected.id !== SELECTED_TOP_NUM.NUM_SELF_CONFIG) {
            this.okTypeOptionIndex = this.okIfaceSelected.id === SELECTED_TOP_NUM.NUM_TOP05 ? 0 : 1;
            const indexData = this.okIfaceSelected.id === SELECTED_TOP_NUM.NUM_TOP05 ? 4 : 9;
            this.okIfaceTop[this.okTypeSelected[0].id].name.forEach((item, index) => {
                if (index <= indexData) {
                    const obj = {
                        label: item,
                        id: item
        ...
        if (this.okIfaceCheckedList.length !== 0) {
            this.networkDetail.okIfaceChange({
                data: okIfaceSelectData, show: this.transmission,
                change: this.transmissionInterfaceChange
            });
        }
        ...
        if (this.errorIfaceSelected.id === SELECTED_TOP_NUM.NUM_SELF_CONFIG) {
            this.errorCheckedList.forEach((item) => {
                const obj = {
                    label: item.id,
                    id: item.id
                };
        ...
        } else if (this.errorIfaceSelected.id !== SELECTED_TOP_NUM.NUM_SELF_CONFIG) {
            this.errorTypeOptionIndex = this.errorIfaceSelected.id === SELECTED_TOP_NUM.NUM_TOP05 ? 0 : 1;
            const indexData = this.errorIfaceSelected.id === SELECTED_TOP_NUM.NUM_TOP05 ? 4 : 9;
            this.errorTop[this.errorTypeSelected[0].id].name.forEach((item, index) => {
                if (index <= indexData) {
                    const obj = {
                        label: item,
                        id: item
    ...
    public assignmentId(a: Array<any>, b: Array<any>) {
        const arr: Array<any> = [];
        a.forEach((item, index) => {
            b.forEach((itemData) => {
                if (item.id === itemData) {
     ...
     * 表格必选其一
     */
    public checkboxData(e) {
        if (e === 1) {
            this.cpuCheckedList = this.assignmentId(this.cpuTableOriginal, this.cpuCheckedListKey);
            this.cpuSelectData = this.cpuCheckedList.length;
            if (!this.cpuSelected) {
                this.cpuTotalNumber = this.cpuCheckedList.length;
            }
        ...
        if (e === 2) {
            this.diskCheckedList = this.assignmentId(this.diskTableOriginal, this.diskCheckedListKey);
            this.diskSelectTableData = this.diskCheckedList.length;
            if (!this.diskSelected) {
                this.diskTotalNumber = this.diskCheckedList.length;
            }
        ...
        if (e === 3) {
            this.okIfaceCheckedList = this.assignmentId(this.okIfaceTableOriginal, this.okIfaceCheckedListKey);
        ...
        if (e === 4) {
            this.errorCheckedList = this.assignmentId(this.errorTableOriginal, this.errorCheckedListKey);
     ...
     */
    public closeMask() {
        this.elementRef.nativeElement.querySelector('#timeChartConfig_close').click();
     ...
     * 筛选 鼠标移入
     */
    public mouseenterFilter() {
        this.filterImg = './assets/img/filterHover.svg';
    }
    /**
     * 筛选 鼠标移出
     */
    public mouseleaveFilter() {
        this.filterImg = './assets/img/filterNormal.svg';
    }
}
