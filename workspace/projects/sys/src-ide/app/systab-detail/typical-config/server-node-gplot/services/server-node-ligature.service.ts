...
export class ServerNodeLigatureService {
   ...
   */
  public getTreeLinkPathList(treeNode: TreeNodeRef): string[] {
  ...
  private genBrokenLine(p: [TwoNumber, TwoNumber, TwoNumber], borderRadius: number): string {
    ...
    const p3 = p[1];
    const p5 = p[2];
    const p2 = calcPoint(p3, p1, borderRadius);
    const p4 = calcPoint(p3, p5, borderRadius);

    function calcPoint(a: TwoNumber, b: TwoNumber, m: number) {
      let k;
      const dy = b[1] - a[1];
      const dx = b[0] - a[0];
      if (dx < 0.01 && dy > 0) {
