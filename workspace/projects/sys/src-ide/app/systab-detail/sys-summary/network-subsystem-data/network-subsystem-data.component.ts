...
export class NetworkSubsystemDataComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.networkPortTitle = [
            ...
            {
                title: 'txkB/s',
                tip: this.i18n.sys_cof.sum.pcie_info.txkB_details,
                showDetail: true,
            },
            {
                title: 'rxkB/s',
                tip: this.i18n.sys_cof.sum.pcie_info.rxkB_details,
                showDetail: true,
            },
            {
                title: 'txpck/s',
                tip: this.i18n.sys_cof.sum.pcie_info.txpck_details,
                showDetail: true,
            },
            {
                title: 'rxpck/s',
                tip: this.i18n.sys_cof.sum.pcie_info.rxpck_details,
                showDetail: true,
            },
            {
    ...
    public getNetWorkData(data) {
        ...
        for (const key in data) {
            if (key !== 'suggest') {
                for (const keyName in data[key].network) {
                    if (keyName !== 'specs') {
                        let obj = {};
                        const name = keyName;
        ...
        for (const key in data) {
            if (data[key].rela && key !== 'suggest') {
                if (data[key].rela.config) {
                    for (const tipName of Object.keys(data[key].rela.config)) {
                      let obj = {};
                      const name = data[key].rela.config[tipName].Latency
                        ? data[key].rela.config[tipName].Latency
                        : '--';
                      const rxpck = data[key].rela.config[tipName].NUMAnode
                        ? data[key].rela.config[tipName].NUMAnode
                        : '--';
                      const txpck = data[key].rela.config[tipName]
                        .Kerneldriverinuse
                      ...
                      const rxkB = data[key].rela.config[tipName].Kernelmodules
                        ...
                        : '--';
                      const txkB = data[key].rela.config[tipName]
                        .Systemperipheral
