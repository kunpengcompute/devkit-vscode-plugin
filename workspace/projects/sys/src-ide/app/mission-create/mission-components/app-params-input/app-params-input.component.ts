import {
  ...
  SecurityContext, ChangeDetectionStrategy, ChangeDetectorRef
} from '@angular/core';
import { DomSanitizer } from '@angular/platform-browser';
import { AppAndParams } from './../../mission-domain/index';
...
export class AppParamsInputComponent implements OnInit, ControlValueAccessor {
  ...
  ngOnInit() {
    // 获取 应用路径
    this.vscodeService.get({url: '/config/system/'}, (resp: any) => {
      this.modeAppPathAllow = resp.TARGET_APP_PATH == null ? '' : resp.TARGET_APP_PATH;
  ...
  public missionAppChange(e: any): any {
    this.isFirstFocusAppPathInput = false;
     // 去掉字符串的头尾空格
    e = this.modeApplication ? this.modeApplication.toString().trim() : '';
