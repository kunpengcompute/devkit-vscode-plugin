...
export class MissionDiagnosisLaunchComponent implements OnInit {
  ...
  @Input() appPathTip: string;
  @Input() diagnoseContent: number;
  ...
  public confirm(context: any) {
    ...
    params = !this.isAtttach
      ? Object.assign(params, {
        ...
        runUser: {
          user_name: this.userFormGroup.get('runUserStatus').value ? this.userFormGroup.get('user').value : '',
  ...
  public open(nodeParams: any) {
    ...
    if (Object.keys(nodeParams.param).length !== 0) {
      ...
      if (!this.isAtttach) {
        ...
        this.userFormGroup.get('runUserStatus').setValue(nodeConfigedData?.runUserSwitch);
        this.userFormGroup.get('user').setValue(nodeConfigedData?.runUserSwitch ? nodeConfigedData?.runUser.user_name
          || nodeParams.runUser.user_name : '');
        this.userFormGroup.get('password').setValue(nodeConfigedData?.runUserSwitch ? nodeConfigedData?.runUser.password
          || nodeParams.runUser.password : '');
      } else {
        let processPid = '';
        let processName = '';
    ...
    if (!nodeConfigedData?.runUserSwitch) {
      this.userFormGroup.reset();
    }
    setTimeout(() => {
      this.nodeTiModal = this.tiModal.open(this.nodeConfigModal, {
        modalClass: 'nodeConfigModals',
        draggable: false
      });
    }, 0);
  }
