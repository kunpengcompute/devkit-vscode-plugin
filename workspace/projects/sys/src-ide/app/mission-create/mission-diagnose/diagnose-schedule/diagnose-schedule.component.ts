...
export class DiagnoseScheduleComponent implements OnInit, AfterViewInit {
  @ViewChild('diagnoseAttach') diagnoseAttach: any;
  ...
  public sureUpdata() {
    ...
    const option = {
      url: `/schedule-tasks/${this.scheduleTaskId}/`,
      params,
    };
   ...
   */
  public getAppDir() {
    this.vscodeService.get({ url: '/config/system/?analysis-type=memory_diagnostic' }, ({ data }: any) => {
    ...
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (!isIncluded) {
        const modeAppWarnMsg = (this.i18n.mission_create.modeAppPathInvalid as string)
          .replace('${path}', this.vscodeService.getPathString(this.modeAppPathAllow));
