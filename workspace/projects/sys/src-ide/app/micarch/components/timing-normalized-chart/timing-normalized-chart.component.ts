import { Component, OnInit, AfterViewInit, EventEmitter, Input, ElementRef, Output, ViewChild } from '@angular/core';
...
export class TimingNormalizedChartComponent implements OnInit, AfterViewInit {
    ...
    @Output() pointerMove = new EventEmitter<{
        data: any, pos: { left: number, top: number },
        posInChart?: { left: number, top: number }
    }>();
    ...
    private renderChart({ data, size, color, domain, columns, cd }) {
        const componentCtx: TimingNormalizedChartComponent = this;
        const clipId = 'clip-' + Utils.generateConversationId(20);
        const height: number = size.height;
        const width: number = size.width;
        let currentXScale: any;
        const stackHoverColor = 'rgba(44, 142, 139, 1)';
