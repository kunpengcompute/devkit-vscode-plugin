import { Component, OnInit, AfterViewInit, AfterViewChecked, ElementRef, Input, ViewChild } from '@angular/core';
...
export class TimingNormalizedAxisComponent implements OnInit, AfterViewInit, AfterViewChecked {
    ...
    constructor(
        ...
        this.format = (num: number) => this.axios.setThousandSeparator(num + 'ms');
    }
    ...
    ngAfterViewChecked(): void {
        if (this.axisSelection
            ...
            && this.xAxisEntity
            && this.option) {
            if (this.currentAxisWidth === 0 && this.axisSelection.width() > 0) {
                this.xAxisEntity.amend();
            }
     ...
     * renderXAxis
     */
    public renderXAxis({ size, domain }): { svg: any, transform: (d: any) => void, amend: () => void } {
