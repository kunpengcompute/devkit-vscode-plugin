...
export class ConfigDataComponent implements OnInit {
  ...
  public getData() {
    const params = { queryType: 'summary', taskId: encodeURIComponent(this.taskid) };
  ...
  public viewDetails(el: {
    type: string; title: string; key: any; clicked: boolean; prop?: string; columns?: Array<TiTableColumns>
  }) {
    this.isLoading = true;
    this.cleanData();
    el.clicked = true;
    ...
    } else if (this.tableType === 'multipleObjects') {
      this.multipleObjectsColumns = JSON.parse(JSON.stringify(el.columns));
  ...
  public dealDifferentNodes(res: any) {
    ...
    this.differentNodesData = resData.map((val: any, idx: number) => {
      ...
      const item: { [x: string]: any } = {
        srcData, expand, displayed, diffrent: val.different || false, projectName: val.title
      };
