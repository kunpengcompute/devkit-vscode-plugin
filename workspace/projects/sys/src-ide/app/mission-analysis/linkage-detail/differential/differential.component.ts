...
export class DifferentialComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  ngOnInit() {
    ...
    { title: this.i18n.diffflamegraph.search_mark, color: '#8739db', show: true, key: 'search mark' }];
  ...
  public initSvg() {
    ...
    let fireUrl = `/tasks/taskcontrast/hotspot/diff-flame/?`;
    const params = this.flameInfo;
    this.obtainingFlameData = true;
    fireUrl += Utils.converUrl(params);
    this.disabled = true;
    this.vscodeService.get({ url: fireUrl }, (resp: any) => {
      this.disabled = false;
   ...
   */
  setColor = (d: any) => {
    return d.highlight ? '#8739db' : this.flamegraphService.colorHash(d);
  }
  public update() {
