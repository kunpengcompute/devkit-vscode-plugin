import {
    Directive, Input, ElementRef, Renderer2, AfterViewInit, TemplateRef, EmbeddedViewRef, ApplicationRef
} from '@angular/core';
...
export class FollowTooltipDirective implements AfterViewInit {
    ...
    set tooltipInfo(val: any) {
        ...
        if (this.tooltipElement) {
            this.setContent(
                this.flagTooltipInfoCopy.html,
                this.flagTooltipInfoCopy.context
            );
            setTimeout(() => {
    ...
    private createElement() {
        ...
        this.renderer.setStyle(
            this.tooltipElement,
            '-webkit-transition',
            `opacity 500ms ease 0ms, left 500ms linear 0ms`
        );
        this.renderer.setStyle(
            this.tooltipElement,
            '-moz-transition',
    ...
    private setContent(content: string | TemplateRef<any>, context?: any) {
        ...
        if (content instanceof TemplateRef) {
            const embeddedView: EmbeddedViewRef<any> = content.createEmbeddedView(
                { context }
            );
