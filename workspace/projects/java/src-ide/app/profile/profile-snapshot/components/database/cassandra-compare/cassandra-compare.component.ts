...
export class CassandraCompareComponent implements OnInit {
    ...
    @ViewChild('echartsModel', { static: false }) echartsModel: any;
    ...
    public intellijSnapshotA: any;
    public intellijSnapshotB: any;
    ...
    ngOnInit(): void {
        ...
        this.setHeapLabel(this.currentHeapLabel, this.prevHeapLabel);
    ...
    public getData(currentHeapLabel: any, prevHeapLabel: any) {
        ...
        this.setHeapLabel(currentHeapLabel, prevHeapLabel);
        this.snapShot = JSON.parse(this.downloadService.downloadItems.snapShot.snapShotData);
        this.snapShotData(this.snapShot);
        this.handleSortCompare(this.snapShotADataFile, this.snapShotBDataFile);
        this.comparedDatas = this.handleFileOriData(this.snapShotADataFile, this.snapShotBDataFile);
     ...
     * 设置intellij名称
     */
    setHeapLabel(currentHeapLabel: string, prevHeapLabel: string){
      if (this.isIntellij){
        this.intellijSnapshotA = this.formatDate(prevHeapLabel) + ' ' + prevHeapLabel.split(' ')[1];
        this.intellijSnapshotB = this.formatDate(currentHeapLabel) + ' ' + currentHeapLabel.split(' ')[1];
      }
    }
    /**
     * 格式化日期
    ...
    public handleFileOriData(base: any, compare: any) {
        ...
        base.forEach((item: any) => {
            const compareFile = compare.find((compareData: any): any => {
                if (Object.prototype.hasOwnProperty.call(compareData, 'label')) {
                    return compareData.label === item.label;
                }
            });
            if (!compareFile) {
                compare.push(item);
            ...
            for (const key in compareFile) {
                if (Object.prototype.hasOwnProperty.call(compareFile, key)) {
                    ...
                    if (typeof compareFile[key] === 'string') {
                        if (key === 'aver') {
                            compareFile.compare[key] = (Number(compareFile[key]) -
                             Number(item[key])).toFixed(2).toString();
                        }
                    }
                }
            }
    ...
    public toggleSnapshot() {
        ...
        if (this.snapshotA === this.prevHeapLabel) {
            ...
            this.setHeapLabel(this.prevHeapLabel, this.currentHeapLabel);
    ...
    private getCountProportion() {
        ...
        this.data.forEach((item: any) => {
            ...
            item.totalDurPer = item.duration / 1000 + '(' + ((item.duration /
               this.BdurationTotal) * 100).toFixed(2) + '%)';
            item.compare.totalDur = item.compare.duration / 1000;
            item.compare.totalPer = (((item.duration / this.BdurationTotal) - item.compare.percentage)
                / item.compare.percentage * 100).toFixed(2);
    ...
    public expandTable() {
        ...
        setTimeout(() => {
            ...
            this.tableContainer.style.height = this.expandFlag ? '600px' : '320px';
        }, 50);
    ...
    private getTreeTableArr(pArray: Array<any>, pLevel?: number, pId?: any): Array<any> {
        ...
        for (const item of pArray) {
            ...
            temp.totalDurPer = temp.duration / 1000 + '(' +
             ((temp.duration / this.BdurationTotal) * 100).toFixed(2) + '%)';
            temp.compare.totalDur = temp.compare.duration / 1000;
            if (item.compare.percentage === 0) {
                temp.compare.totalPer = 0;
            } else {
                temp.compare.totalPer = (((item.duration / this.BdurationTotal) - item.compare.percentage) /
                 item.compare.percentage * 100).toFixed(2);
            }
    ...
    public replaceCompared(level: any, item: any, compared: any) {
        for (const node of item) {
            ...
            if (node.children) {
                this.replaceCompared(level, node.children, compared);
            }
        }
    }
    ...
    *@param  查询所需要的id
    */
    private getObjById(level: any, list: any[]): any {
        // 遍历数组
    ...
    private snapDataFile(snapShotData: any) {
