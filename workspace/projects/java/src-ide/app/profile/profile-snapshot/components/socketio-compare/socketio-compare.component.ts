...
@Component({
    selector: 'app-socketio-compare',
    templateUrl: './socketio-compare.component.html',
    styleUrls: ['./socketio-compare.component.scss']
})
export class SocketioCompareComponent implements OnInit {
    ...
    @ViewChild('socketioTimeLine', { static: false }) socketioTimeLine: any;
     ...
     * 搜索
     */
    public searchEvent(event: any, type: string): void {
        if (type === 'left') {
    ...
    private sortAOrB(data: any, AOrB: any) {
        ...
        data.forEach((item: any) => {
            ...
            } else if (Object.prototype.hasOwnProperty.call(item, 'threads')) {
                if (Array.isArray(item.threads)) {
                    this.sortAOrB(item.threads, AOrB);
                }
            }
        });
        return data;
        ...
        base.forEach((item: any) => {
            ...
            if (!compareFile) {
                ...
                if (Object.prototype.hasOwnProperty.call(compareFile, 'fd')) {
                    compareFile.threads = this.threadCompare(item.threads, compareFile.threads);
                }
            }
    ...
    public handleUpdateThreads(fileName: any) {
        ...
        fileName.children.forEach((host: { children: any[] }) => {
            host.children.forEach((fd) => {
                threadsList = threadsList.concat(fd.threads);
            });
        });
     ...
     * @param dura dura
     */
    public onChangeRate(byte: any, dura: number) {
        if (byte === 0) {
            return 0;
        }
        if (dura === 0) {
     ...
     * 点击表格行
     */
    public handleFileClicked(ip: any, i: any) {
        this.handleIsNowEcharts('ip', ip.ip);
        this.handleUpdateThreads(ip);
    ...
    public handleFdClicked(fd: any, i: any, idx: any) {
        ...
        this.srcDataTableTime.data = fd.threads;
        this.dataTableTime = fd.threads;
        this.onTypeSelect();
        this.onClickRightTable(this.srcDataTableTime.data[0], 0);
     ...
     * @param host 当前host数据
     */
    public handleUpdateHostThreads(host: { children: any[] }) {
        let threadList: any[] = [];
        host.children.forEach((fd) => {
            threadList = threadList.concat(fd.threads);
        });
     ...
     */
    public handleHostSelected(i: any, idx: number) {
        this.srcDataTable.data[i].children.forEach((item: any, index: number) => {
            if (index !== idx) {
                item.showDetails = false;
            }
            item.isSelect = index === idx;
        });
        this.srcDataTable.data[i].children[idx].children.forEach((item: { isSelect: boolean }) => {
            item.isSelect = false;
        });
    }
    /**
    ...
    public beforeToggleSecond(row: TiTableRowData): void {
        this.currentHostTableListTop.forEach((e) => {
            if (row.host !== e.host) {
     ...
     * @param idx 本条数据所在位置
     */
    public handleFdSelected(i: any, idx: number) {
    ...
    public handleEchartsCompare() {
        ...
        this.compareEchartsData.timeList = [...new Set(
          this.compareEchartsData.timeAList.concat(this.compareEchartsData.timeBList))];
    ...
    public initEchart() {
        ...
        setTimeout(() => {
            ...
            this.fileIOEcharts.on('datazoom', (params: { batch: any }) => {  // 放大缩小时调用接口
                this.socketioTimeLine.dataConfig({ start: params.batch[0].start, end: params.batch[0].end });
            });
        }, 0);
    }
    /**
