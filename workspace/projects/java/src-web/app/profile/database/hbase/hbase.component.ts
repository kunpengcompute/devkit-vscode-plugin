...
export class HbaseComponent implements OnInit, OnDestroy, AfterContentInit {
  ...
  @ViewChild('hbaseEcharts') hbaseEcharts: any;
  ...
  private hbaseSub: Subscription;
  private updataHbaseSub: Subscription;
  ...
  ngOnInit() {
    this.limitTime = this.downloadService.dataLimit.hbase.timeValue;
    this.noDataMsg = this.i18n.profileNoData.hBaseNoDataMsg;
    this.srcData = {
      data: [], // 源数据
      state: {
        searched: false,
    ...
    if (this.isDownload) {
      this.hbaseGroup.controls.hbase_threshold.setValue(
        this.downloadService.downloadItems.hbase.threshold);
    ...
    this.hbaseSub = this.msgService.getMessage().subscribe(msg => {
      if (msg.type === 'hbase') {
        if (msg.data) {
          this.clearHbaseTimer();
        }
      ...
      if (msg.type === 'isRestart') {
        ...
        this.hbaseGroup.controls.hbase_threshold.setValue(50);
        this.snapCount = 0;
        this.clearHbaseTimer();
        this.initData();
      }
    ...
    this.updataHbaseSub = this.msgService
      .getMessage()
      .subscribe(msg => {
  ...
  private merge(theTree: any, frames: any, itemData: any) {
    ...
    frames.forEach((frame: any) => {
      ...
      if (!sameChild) {
        const pId = node.id || itemData.hql;
        sameChild = {
          label_name: desc,
          label: '',
          count: 0,
  ...
  private updateData(data: any) {
    ...
    if (nowDate - this.lastRefreshEchartTime >= 2000) {
      ...
      if (this.hbaseEcharts) {
        this.hbaseEcharts.updateEchartsData(this.updateOptions);
      }
    }
  }
  ...
  startHbase() {
    ...
    this.stompService.hbaseTimeInit = {
      startTime: 0,
      endTime: 0,
      count_pre_s: 0,
      duration_pre_s: 0
    };
    if (this.hbaseGroup.controls.hbase_threshold.value === '') {
    ...
    if (this.srcData.data.length === 0) {
      this.hbaseTimeout = setTimeout(() => {
        this.mytip.alertInfo({
          type: 'warn',
          content: this.i18n.profileNodataTip.hbase,
          time: 10000
        });
      }, 30000);
    }
