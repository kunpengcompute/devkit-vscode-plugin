...
export class StructAssignComponent implements OnInit, AfterViewInit {
    ...
    ngAfterViewInit(): void {
        for (let i = 0; i < this.struckData['32-bits'].length; i++) {
            this.struckData['32-bits'][i].struct_type = this.i18n.plugins_porting_message_enhancefun_struct.bitOf32;
            this.struckData['64-bits'][i].struct_type = this.i18n.plugins_porting_message_enhancefun_struct.bitOf64;
        ...
        for (let i = 0; i < a.length; i++) {
            const element = a[i];
            if (a.eq(i).text() === this.i18n.plugins_porting_message_enhancefun_struct.bitOf32) {
                a.eq(i).parent().parent().append('<div class="siblings"></div>')
                  .find('.siblings').attr('id', 'siblings' + i);
                if (i !== 0) {
    ...
    public hierarchy(list: any) {
        ...
        let maxDepth = 0;
        let breadth = 0;
        function recursive(that: any, item: any, childList: any) {
            let childrenNum = 0;
            depth++;
            const node: any = {
                col: depth,
                ...
                children: [],
                value: item,
                id: that.utilsService.generateConversationId(15),
                lineId: that.utilsService.generateConversationId(15)
            };
            childList.push(node);
            if (maxDepth < depth) {
                maxDepth = depth;
            ...
            item.suggestion_content.forEach((element: any) => {
                if (Object.keys(element.value_node).length > 0) {
                    ...
                    recursive(that, element.value_node, node.children);
                }
            });
            depth--;
        }
    ...
    public renderCell(item: any, id: string, lineId: string) {
      ...
      <div class="struck-value"
        style="${item.value.suggestion_content.length === (idx + 1)
          ? 'border-bottom:1px solid var(--font-d-616161-l-979797)'
          : null};"
        id=${Object.keys(elem.value_node).length !== 0
          ? ('b' + (index - 1) + lineId)
          : null}>
      ...
      ${elem.value_name}
      <span style="padding-right:13px;display:${elem.value_size === 0 ? 'none' : null}">:${elem.value_size}B</span>
      </span>
                  </div>
                  <div class="struck-value"
                    style="font-size:14px;
                    background-color:var(--bg-d-424242-l-ffeeee);
                    display:${!elem.byte_hole ? 'none' : null};
                    border: 1px solid var(--border-d-888888-l-979797);
                  ${item.value.suggestion_content.length === (idx + 1)
                    ? 'border-bottom:1px solid var(--border-d-888888-l-979797)'
                    : null}">
    ...
    <div class="struck-box" id=${'a' + id}>
      ${item.col === 1 ? title : ''}
      <span class="struck-title" id=${id} title = ${item.value.struct_name.length > 18 ?
                `${item.value.struct_name}` : ''}>${item.value.struct_name}</span>
      ${contentHtml}
    </div>
    <script>
    ...
    <style>
      .struck-value:last-child {
          border-bottom: 1px solid var(--border-d-616161-l-ffffff);
      }
    </style>`;
        return struckHtml;
    }
        ...
        function recursive(node: any) {
          ...
          node.children.forEach((element: any, idx: any) => {
            if (edit !== 'scroll' && edit !== 'delete') {
              ...
              const leaderLine =
                that.drawConnector($('#b' + idx + node.lineId).eq(0), $('#' + element.id).eq(0), 'struck');
    ...
    fixPosition(line: any) {
        ...
        if (translate.x !== 0 || translate.y !== 0) {
            ...
            this.lineList.forEach((lineItem: any) => { lineItem.position(); });
        } else if (line) {
