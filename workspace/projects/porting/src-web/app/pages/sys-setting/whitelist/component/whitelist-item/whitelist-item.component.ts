...
export class WhitelistItemComponent implements OnInit {
  ...
  public whitelistUrl = './api/portadv/tasks/dependency_dictionary/package/';
  ...
  onCompleteItems($event: any): void {
    ...
    if (
      ...
      setTimeout(() => {
        $('#uploadId_white .ti3-aui-file-state-general-success').addClass('errorTip');
  ...
  addItemSuccess_white(fileItems: TiFileItem): void {
    const fileName = fileItems._file.name;
    const size = fileItems._file.size;
    this.Axios.axios.get(`/portadv/tasks/dependency_dictionary/package/?filename=${fileName}&filesize=${size}`)
    .then((resp: any) => {
      if (this.commonService.handleStatus(resp) === 0) {
        ...
        return;
      } else if (this.commonService.handleStatus(resp) === 1 && !resp.data.status) {
        this.canUpgrade = false;
        ...
        const info = sessionStorage.getItem('language') === 'zh-cn' ? resp.infochinese : resp.info;
        this.deleteWhiteModal.title = info;
        return;
      }
      this.addItemValidFailed(resp, 'white');
    });
  ...
  public confirmHandle(flag: any) {
    if (flag) {
      this.Axios.axios.post(`/portadv/tasks/delete/dependency_dictionary/package/`)
      .then((resp: any) => {
        if (this.commonService.handleStatus(resp) === 0) {
          const tipEl = $('#uploadId_white .ti3-aui-file-state-general-info');
          const uploadBtn = $('#uploadId_white .ti3-aui-upload-btn');
          tipEl.html('');
          uploadBtn.removeAttr('disabled');
          return;
        ...
        const info = sessionStorage.getItem('language') === 'zh-cn' ? resp.infochinese : resp.info;
        this.mytip.alertInfo({ type: 'warn', content: info, time: 10000 });
      });
      return;
    }
  }

  private addItemValidFailed(resp: any, type: any) {
